...
st.title("Menu Item Sales Per Country for February 2022")
...
def load_data():
    ...
    sales_df = session.table("tb_101.analytics.menu_item_sales_feb_2022").to_pandas()
    return sales_df
...
# Get a unique list of country and menu item names from the DataFrame to populate the dropdown.
country_names = sales['COUNTRY'].unique().tolist()
menu_item_names = sales['MENU_ITEM_NAME'].unique().tolist()
...
selected_countries = st.selectbox("Select a country", options=country_names)
...
menu_item_sales = sales[
    (sales['COUNTRY'] == selected_countries) &
    (sales['MENU_ITEM_NAME'] == selected_menu_item) 
]
...
####### playing around ##########
# --- Data Setup ---
# Country-only filter (so we can compare items within the selected country)
menu_item_sales_total = sales[sales['COUNTRY'] == selected_countries]

# Group by item within that country
menu_item_totals = (
    menu_item_sales_total
    .groupby('MENU_ITEM_NAME', as_index=False)['ORDER_TOTAL']
    .sum()
...
bar_chart = (
    alt.Chart(menu_item_totals)
      .mark_bar()
      .encode(
          y=alt.Y('MENU_ITEM_NAME:N', sort='-x', title='Menu Item'),
          x=alt.X('ORDER_TOTAL:Q', title='Total Sales ($)'),
          tooltip=[
              alt.Tooltip('MENU_ITEM_NAME:N', title='Menu Item'),
              alt.Tooltip('ORDER_TOTAL:Q', title='Total Sales', format=',.2f')
          ]
      )
      .properties(
          height=700,
          title=f'Total Sales per Menu Item — {selected_countries}'
      )
)
